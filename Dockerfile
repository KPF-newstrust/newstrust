...
ADD worker/mecab/mecab-0.996-ko-0.9.2.tar.gz /opt/mecab
...
ADD worker/mecab/mecab-ko-dic-2.1.1-20180720.tar.gz /opt/mecab
RUN cd /opt/mecab/mecab-ko-dic-2.1.1-20180720 && \
	./autogen.sh && \
	ldconfig && \
	./configure && \
	make && \
...
RUN apt-get -y install vim-tiny wget
