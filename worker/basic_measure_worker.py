...
def on_task(raw_body):
    helper.eventlog.debug("Got task %s" % raw_body)
    try:
        task_spec = json.loads(raw_body.decode("utf-8"))
        cmd = task_spec["cmd"]
    ...
    try:
        ...
        # elif cmd == "lab_split": #문장분리
            # handler_lab.process_split(ver, task_spec)
        elif cmd == "lab_postag": #형태소분석
        ...
        elif cmd == "lab_trust": #저널리즘가치
            handler_lab.process_trust(ver, task_spec)
        elif cmd == "lab_integrate": #통합평가
            handler_lab.process_integrate(ver, task_spec)
        elif cmd == "dic_custom": #형태소사전 업데이트
            handler_dic.publish_userdic()
        elif cmd == "crawling_process": #크롤링기사 처리
            handler_crawling.process()
        else:
...
#     coll_stat = helper.mongo.get_collection_nt("asStats")
    # docs = coll_stat.find({})
    # for doc in docs:
...
#     coll_news = helper.mongo.get_collection_nt("news")
#     docs = coll_news.find({})
#     for doc in docs:
