...
def get_metric_update_dicts(params):
    ...
    # helper.eventlog.trace("Mecab: %f seconds: %s" % (news_entity["mecab_time"], news_entity["mecab_postag"]))
    ...
    # helper.eventlog.trace("Hannanum: %f seconds: %s" % (news_entity["hannanum_time"], news_entity["hannanum_postag"]))
    ...
    # helper.eventlog.trace("Kkma: %f seconds: %s" % (news_entity["kkma_time"], news_entity["kkma_postag"]))
    ...
    # helper.eventlog.trace("Twitter: %f seconds: %s" % (news_entity["twitter_time"], news_entity["twitter_postag"]))
    ...
    try:
        ...
        print("주관적술어/무주체술어 추출 에러 : PASS (%s)" % str(ex))
        helper.eventlog.debug("주관적술어/무주체술어 추출 에러 : PASS (%s)" % str(ex))
        pass
...
def process_basic_measurements(ver, param):
    ...
    coll_src = helper.mongo.get_collection_nt(COLLNAME_NEWS_SRC)
    coll_dst = helper.mongo.get_collection_nt(COLLNAME_NEWS)
    coll_ett = helper.mongo.get_collection_nt(COLLNAME_NEWS_ENTITY)
